...
type
  ...
  private
    ...
    procedure CheckTimeOutConnection;
  protected
    procedure Execute; override;
  public
    constructor Create;
...
{$ELSE}
    ...
    SendData.WriteEx(TPacketType(Packet.PacketType), SizeOf(TPacketType));
    SendData.WriteEx(Int64(Packet.Id), SizeOf(Int64));
    SendData.WriteEx(Integer(Packet.OrderId), SizeOf(Integer));
    SendData.WriteEx(Boolean(Packet.Confirm), SizeOf(Boolean));
    SendData.WriteEx(Integer(Packet.TotalSize), SizeOf(Integer));
    SendData.WriteEx(Integer(Packet.CurSize), SizeOf(Integer));
    SendData.Write(Packet.Data[0], Packet.CurSize);
{$IFEND}
    SendData.Position := 0;
    ...
    try
      ...
      FSocket.SendBuffer(Connection.RemoteAddr, Connection.RemotePort, TIdBytes(DataBytes));
    finally
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              Connection.DisconnectTick := TDSMisc.TickCount;
...
procedure TDSHeartBeatFrame.CheckTimeOutConnection;
var
  I: Integer;
  Connections: TDSConnectionsEx;
  Indexs: TIndexs;
  ...
  try
    ...
    begin
      ...
      try
        ...
        for I := 0 to Connections.Count - 1 do
        begin
          if Connections.Items[I].Disconnect then
          begin
            Indexs.Add(I);
          end
          else if (Connections.Items[I].Tick + UDP_TIME_OUT) <= TDSMisc.TickCount then
          begin
            Indexs.Add(I);
          end;
        end;
...
{$ELSE}
    SendData.WriteEx(Integer(UDP_HEAD), SizeOf(Integer));
    SendData.WriteEx(TPacketType(pt_HeartBeat), SizeOf(TPacketType));
    SendData.WriteEx(Int64(0), SizeOf(Int64));
    SendData.WriteEx(Integer(0), SizeOf(Integer));
    SendData.WriteEx(Boolean(False), SizeOf(Boolean));
    SendData.WriteEx(Integer(0), SizeOf(Integer));
    SendData.WriteEx(Integer(0), SizeOf(Integer));
